...
if [ -f "$final_results_path/charmer_10.csv" ]; then
    echo "Charmer 10 already exists, skipping..."
else
    python attack.py \
        --device cuda \
        --loss margin \
    ...
    mv "$result_path/${model_name}_10${charmer_ending_path}" "$final_results_path/charmer_10.csv"
fi