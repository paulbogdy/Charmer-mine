...
if [ -f "$final_results_path/textfooler.csv" ]; then
    echo "textfooler already exists, skipping..."
else
    python attack.py \
        --device cuda \
        --loss margin \
    ...
    mv "$result_path/${attack}_${dataset}_${model_name}.csv" "$final_results_path/textfooler.csv"
fi